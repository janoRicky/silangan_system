        ...
        try {
            ...
            if (count($this->attendance_data) > 0) {
                ...
                while (strlen($attendance_data) > 40) {
                    ...
                    $aid = $u1 + ($u2 * 256);
                    $uid = str_replace("\0", '', hex2bin(substr($u[1], 8, 16)));
                    $v_type = hexdec(substr($u[1], 56, 2));
                    $timestamp = $this->decodeTime(hexdec($this->reverseHex(substr($u[1], 58, 8))));
                    $v_state = hexdec(substr($u[1], 66, 2));
                    array_push($attendance, array("aid"=>$aid, "uid"=>$uid, "v_type"=>$v_type, "timestamp"=>$timestamp, "v_state"=>$v_state));
